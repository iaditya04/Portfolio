...
const contactCards = document.querySelectorAll("#contact .contact-info-container");
  ...
  const contactObserver = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        // Footer fade-in after contact section
        if (footer) {
          setTimeout(() => {
            footer.classList.add("in-view");
          }, contactCards.length * 150 + 200);
        }

        contactObserver.unobserve(contactSection); // run once
      }
    });
  }, { threshold: 0.3 });
